...
- Python **3.12+**
- [`uv`](https://docs.astral.sh/uv/) for Python dependency management and the local launcher
- Node.js **22+** for yt-dlp's JavaScript runtime; npm when rebuilding embedded frontend surfaces or the MCP server
...
The launcher runs `uv run --locked uvicorn main:app --host 127.0.0.1 --port 8000`
using the dependencies pinned in `uv.lock`, including yt-dlp's matching
[JavaScript solver](https://github.com/yt-dlp/yt-dlp/wiki/EJS).
`YTDLP_JS_RUNTIME` defaults to `node`; use `node:/absolute/path/to/node`
when Node is outside the service's `PATH`.
...
> `pyproject.toml` declares the project dependencies and `uv.lock` pins their resolved versions. The repository also contains a `requirements.txt` compatibility path used by existing scripts and CI.
...
| `YTDLP_JS_RUNTIME` | yt-dlp JavaScript runtime; defaults to `node`, or use `node:/absolute/path/to/node` |
